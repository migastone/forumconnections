/* ─────────── Meta Pixel: eventi e passaggio dei parametri al checkout ───────────
   ...
   string del link. Per questo il codice del pixel qui in pagina non basta da
   solo: senza questo file la vendita risulta "diretta" e la campagna che l'ha
   generata non prende il merito.
...
(() => {
  ...
  let salvati = {};
  try { salvati = JSON.parse(sessionStorage.getItem(STORE) || '{}'); } catch (e) {}
  ...
  /* URL corrente se c'e', altrimenti la memoria di sessione. */
  const valore = k => (inArrivo.get(k) || '').trim() || salvati[k] || '';
  ...
  /* _fbp invece non si ricostruisce: non deriva da nulla che sia gia'
     nell'URL, e inventarlo qui significherebbe mandare al checkout un id
     diverso da quello che il pixel usa negli eventi del browser. Se il
     cookie non c'e' ancora, il parametro semplicemente non parte. */
  ...
  /* Al clic vero: id nuovo, evento al pixel con quell'eventID e lo stesso id
     nella query string. Il server a valle vede due volte lo stesso acquisto
     (browser e server) e con mm_eid sa che e' uno solo. */
  document.addEventListener('click', e => {
    ...
    try {
      if (window.fbq) fbq('track', 'InitiateCheckout', datiEvento(sku), { eventID: eid });
    } catch (err) {}
  }, true);
  ...
  const avviaOra = () => {
    aggiorna();
    if (skuVC) {
      try { if (window.fbq) fbq('track', 'ViewContent', datiEvento(skuVC)); } catch (e) {}
    }
  };

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', avviaOra);
  } else {
    avviaOra();
  }
})();
