(function(jQuery) {
	...
	function setupCountdown($countdown) {
		...
		if ( countdown.hasAttribute('data-countdown') && countdown.getAttribute('data-countdown') === 'fixed' ) {
			...
			var localStorageVariable = 'countdown-' + ID + (
				jQuery('.js_kartra_trackable_object[data-kt-type="kartra_page_tracking"]').length ?
				jQuery('.js_kartra_trackable_object[data-kt-type="kartra_page_tracking"]').attr('data-kt-value') : ''
			);

			if( localStorage.getItem(localStorageVariable) !== null ) {
				firstVisitStamp = Number(localStorage.getItem(localStorageVariable));
			} else if (localStorage.getItem('countdown-' + ID ) !== null) {
				firstVisitStamp = Number(localStorage.getItem('countdown-' + ID ));
				localStorage.setItem(localStorageVariable, firstVisitStamp);
			} else {
				//now is first visit
				firstVisitStamp = Date.now()/1000;

				//store first visit date
				localStorage.setItem(localStorageVariable, Math.ceil(firstVisitStamp));
			}
